...
describe('handleInitialize', () => {
  test('success', () => {
    ...
    handleInitializeHelper(INITIALIZE_EVENT, TEST_CONFIG, TEST_SPRY_HOOK_ADDRESS)
    ...
    assertObjectMatches('Pool', USDC_WETH_POOL_ID, [
      ['token0', token0.id],
      ['token1', token1.id],
      // dynamic pools start feeTier at the tier base fee, never the 0x800000 sentinel
      ['feeTier', '500'],
      ['tickSpacing', INITIALIZE_FIXTURE.tickSpacing],
      ['hooks', INITIALIZE_FIXTURE.hooks],
      // Spry tier derived from tickSpacing 10 -> LIKE_ASSET (base 0.05%, cap 1.00%)
      ['tier', 'LIKE_ASSET'],
      ['baseFeePips', '500'],
      ['capFeePips', '10000'],
      ['isDynamicFee', 'true'],
