...
import { DYNAMIC_FEE_FLAG, tierFromTickSpacing } from '../../src/utils/spry'
...
// Fixture SpryHook address used so the Initialize filter creates the test pools.
export const TEST_SPRY_HOOK_ADDRESS = '0x1111111111111111111111111111111111111111'
...
// Spry pools are dynamic-fee: their PoolKey.fee is the 0x800000 sentinel (8388608).
...
export const invokePoolCreatedWithMockedEthCalls = (
  ...
  const hooksAddress = Address.fromString(TEST_SPRY_HOOK_ADDRESS)
  const parameters = [
  ...
  handleInitializeHelper(initializeEvent, subgraphConfig, TEST_SPRY_HOOK_ADDRESS)
}
...
export const createAndStoreTestPool = (poolFixture: PoolFixture): Pool => {
  ...
  // Spry tier + dynamic-fee fields (derived from the fixture's tickSpacing)
  const tierDef = tierFromTickSpacing(tickSpacing)!
  pool.tier = tierDef.name
  pool.baseFeePips = tierDef.baseFeePips
  pool.capFeePips = tierDef.capFeePips
  const isDyn = feeTier == DYNAMIC_FEE_FLAG
  pool.isDynamicFee = isDyn
  // mirror Initialize: never store the raw 0x800000 sentinel as feeTier
  pool.feeTier = isDyn ? tierDef.baseFeePips : BigInt.fromI32(feeTier)
