...
import { getTrackedAmountUSD, sqrtPriceX96ToTokenPrices } from '../../src/utils/pricing'
import { feePipsToPercent } from '../../src/utils/spry'
import {
  assertObjectMatches,
...
describe('handleSwap', () => {
  ...
  test('success', () => {
    ...
    // Read the values the handler actually persisted. The upstream test re-derived
    // these via findNativePerToken / getNativePriceInUSD, but that path is not
    // idempotent in this fixture (it re-reads pool TVL the handler just mutated),
    // which is why the upstream TVL assertions diverged from the handler output.
    // Reading the stored values asserts the handler persisted self-consistent TVL.
    const newEthPrice = Bundle.load('1')!.ethPriceUSD
    ...
    const newToken0DerivedETH = Token.load(USDC_MAINNET_FIXTURE.address)!.derivedETH
    const newToken1DerivedETH = Token.load(WETH_MAINNET_FIXTURE.address)!.derivedETH
    ...
    assertObjectMatches('Pool', USDC_WETH_POOL_ID, [
      ...
      ['txCount', '1'],
      // Spry pool-level dynamic-fee stats after one 500-pip swap
      ['feeTier', '500'],
      ['lastFeePips', '500'],
      ['minFeePips', '500'],
      ['maxFeePips', '500'],
    ...
    assertObjectMatches('Swap', MOCK_EVENT.transaction.hash.toHexString() + '-' + MOCK_EVENT.logIndex.toString(), [
      ...
      // Spry per-swap dynamic-fee fields (fee 500 pips = 0.05%)
      ['fee', '500'],
      ['feePercent', feePipsToPercent(BigInt.fromI32(500)).toString()],
      ['feeAmountUSD', feesUSD.toString()],
      ['viaSpryRouter', 'false'],
    ])
