...
| `src/mappings/donate.ts`                   | `Donate` handler (the upstream had none) — records donations on Spry pools.                       |
...
| `src/mappings/modifyLiquidity.ts`     | Removed the aggregator-hook TVL branch — standard V4 TVL path only.                                                                                |
...
automatically scoped to Spry pools — no per-handler address checks needed.
